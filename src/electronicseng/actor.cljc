(ns electronicseng.actor
  "ElectronicsEngineersActor — the ISCO-08 2152 community electronicseng actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one electronicseng operation request
  ...
  The unconditional invariant: the ElectronicsEngineersAdvisor can never
  directly commit a record the ElectronicsEngineersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [electronicseng.advisor :as advisor]
            [electronicseng.governor :as governor]
            [electronicseng.store :as store]))

(defn build-graph
  "Build a compiled ElectronicsEngineersActor graph. `store` implements
  `electronicseng.store/Store`. `advisor` implements
  `electronicseng.advisor/Advisor` (defaults to `mock-advisor`).
